...
#pragma region ServerSaveFramePackage
...
{
	// only store those data on the server
	if (Character == nullptr || !Character->HasAuthority()) return;
	// to have head and tail.
	...
	else
	{
		...
		// tick time interval maybe different,so we may need to remove more than one node.
...
#pragma region ClientHitScoreRequest
...
#pragma region ServerSideRewind(HitScan/Shotgun)
...
#pragma region LineTrancConfirmHit
	...
	const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	TArray<FFramePackage> CachedCurrentFrames;
	...
	for (auto& Frame : FramePackages)
	{
		...
		CachedCurrentFrames.Add(CurrentFrame);
	...
	for (auto& Frame : CachedCurrentFrames)
	{
...
#pragma region GetInterpoletRewindFrame
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime) const
{
	bool bReturn =
...
// actually rewind: move collision boxes back to rewind time
